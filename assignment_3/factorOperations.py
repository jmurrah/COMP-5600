...
def eliminateWithCallTracking(callTrackingList=None):
    def eliminate(factor: Factor, eliminationVariable: str):
        ...
        conditioned = factor.conditionedVariables() - {eliminationVariable}
        input_domain_dict = factor.variableDomainsDict()

        elim_probs = defaultdict(float)
        for assignment_dict in factor.getAllPossibleAssignmentDicts():
            ...
            for var, val in assignment_dict.items():
                if var != eliminationVariable:
                    elim_var_list.append((var, val))
            elim_probs[frozenset(elim_var_list)] += factor.getProbability(
                assignment_dict
            )

        eliminated_factor = Factor(unconditioned, conditioned, input_domain_dict)
        for assignment_dict in eliminated_factor.getAllPossibleAssignmentDicts():
            eliminated_factor.setProbability(
                assignment_dict, elim_probs[frozenset(assignment_dict.items())]
            )
