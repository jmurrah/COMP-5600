...
class BayesNet(object):
    ...
    def __str__(self):
        ...
        netString = (
            "Variables: "
            + ", ".join([str(var) for var in self.__variablesSet])
            + "\n"
            + "Edges: "
            + ", ".join(
                [
                    str(fromVar) + " -> " + str(toVar)
    ...
    def sameValuesDict(self, other):
        if sorted(self.__variableDomainsDict.keys()) != sorted(
            other.__variableDomainsDict.keys()
        ):
            return False
    ...
    def linearizeVariables(self):
        ...
        Useful for sampling.
        """
        inEdgesIncremental = dict(
            [(var, edgeSet.copy()) for (var, edgeSet) in self.__inEdges.items()]
        )
        noIncomingList = [
            var for var in self.__variables if len(self.__inEdges[var]) == 0
        ]
        ...
        hasEdgesLeftOver = any(
            [len(inEdgesIncremental[var]) > 0 for var in self.__variables]
        )
        if hasEdgesLeftOver:
            raise ValueError(
                "Graph has at least one cycle (not a bayes net) "
    ...
    def setCPT(self, variable, CPT):
        ...
        if variable not in self.__variablesSet:
            ...
            if len(unconditionedVariables) != 1:
                raise ValueError(
                    "Unconditioned variables must contain a single element for an entry"
    ...
    def getAllCPTsWithEvidence(self, evidenceDict=None):
        ...
        """
        return [
            self.getCPTWithEvidence(var, evidenceDict) for var in self.__variablesSet
        ]
    ...
    def easierToParseString(self, printVariableDomainsDict=False):
        ...
        for CPT in self.getAllCPTsWithEvidence():
            ...
            returnStrings.append(
                CPT.easierToParseString(
                    prefix=prefix, printVariableDomainsDict=printVariableDomainsDict
                )
            )
...
class Factor(object):
    def __init__(
        ...
        repeatedVariables = set(inputUnconditionedVariables) & set(
            inputConditionedVariables
        )
        if repeatedVariables:
            raise ValueError(
                "unconditionedVariables and conditionedVariables "
                "can't have repeated \n variables. Repeats:\n" + str(repeatedVariables)
            )

        self.__variables = tuple(inputUnconditionedVariables) + tuple(
            inputConditionedVariables
        )  # variables are unique string identifiersk
        self.__variablesSet = set(self.__variables)

        if not self.__variablesSet.issubset(
            set(inputVariableDomainsDict.keys())
        ):  # it's okay for variableDomainsDict to have more items than needed
            raise ValueError(
                "variableDomainsDict doesn't have all the input variables \n"
        ...
        self.__variableDomainsDict = deepcopy(
            inputVariableDomainsDict
        )  # dict that maps {variable : variableDomain}

        self.__variableOrders = dict(
            [(variable, i) for i, variable in enumerate(self.__variables)]
        )  # internal order of the variables
        self.__probDict = (
            {}
        )  # probability values are stored in an {assignmentValuesTuple : probability} dict,
        # since we can't index using assignmentDicts. this is why we have to sort
        products = list(
            itertools.product(
                *[inputVariableDomainsDict[variable] for variable in self.__variables]
            )
        )
    ...
    def __eq__(self, other):
        ...
        """
        variablesEqual = (
            (self.variablesSet() == other.variablesSet())
            and (
                set(self.unconditionedVariables())
                == set(other.unconditionedVariables())
            )
    ...
    def setProbability(self, assignmentDict, probability):
        ...
        if probability < 0:
            raise ValueError(
                "Probabilty entries can't be set to negative values: "
                + str(probability)
            )
        else:
    ...
    def __getAssignmentsInOrder(self, assignmentDict):
        ...
        """
        reducedAssignmentDict = dict(
            [
                (var, val)
                for (var, val) in assignmentDict.items()
                if var in self.__variablesSet
            ]
        )
        variablesAndAssignments = reducedAssignmentDict.items()
        variablesAndAssignments = sorted(
    ...
    def getAllPossibleAssignmentDicts(self):
        ...
        cartesianProductOfAssignments = itertools.product(
            *[
                self.__variableDomainsDict[variable]
                for variable in reversed(self.__variables)
            ]
        )
        return [
            dict(zip(reversed(self.__variables), product))
            for product in cartesianProductOfAssignments
        ]
    ...
    def __str__(self):
        ...
        for variable in self.__variablesSet:
            maxPrintSize = max(
                ...
                max(
                    [
                        len(str(variableValue))
                        for variableValue in self.__variableDomainsDict[variable]
                    ]
                ),
            )
        ...
        varLine = " | " + " | ".join(
            [
                ...
                ].center(printSizeDict[unconditionedVariable], " ")
                for unconditionedVariable in self.__unconditionedVariables
            ]
        )
        if len(self.__conditionedVariables) > 0:
            varLine += " | " + " | ".join(
                [
                    ...
                    ].center(printSizeDict[conditionedVariable], " ")
                    for conditionedVariable in self.__conditionedVariables
                ]
            )
        ...
        if len(self.__conditionedVariables) == 0:
            returnString += (
                " " + "".join(["-" for _ in range(varLineLength - 1)]) + "\n"
            )
        ...
        for assignmentDict in self.getAllPossibleAssignmentDicts():
            ...
            if len(self.__conditionedVariables) > 0:
                conditionedAssignments = [
                    assignmentDict[conditionedVariable]
                    for conditionedVariable in self.__conditionedVariables
                ]
            ...
            returnString += " | " + " | ".join(
                [
                    str(assignmentDict[unconditionedVariable])[
                        : printSizeDict[unconditionedVariable]
                    ].center(printSizeDict[unconditionedVariable], " ")
                    for unconditionedVariable in self.__unconditionedVariables
                ]
            ...
            if len(self.__conditionedVariables) > 0:
                returnString += " | " + " | ".join(
                    [
                        str(assignmentDict[conditionedVariable])[
                            : printSizeDict[conditionedVariable]
                        ].center(printSizeDict[conditionedVariable], " ")
                        for conditionedVariable in self.__conditionedVariables
                    ]
            ...
            if probability is None:
                ...
                formattedProb = (
                    ...
                    else ("%." + str(8 - digits) + "f") % probability
                )
    ...
    def __repr__(self):
        returnRepr = "Factor("
        initArgs = [
            self.__unconditionedVariables,
            self.__conditionedVariables,
            self.__variableDomainsDict,
        ]
    ...
    def easierToParseString(self, prefix=None, printVariableDomainsDict=True):
        ...
        if printVariableDomainsDict:
            ...
            for (key, domain) in self.__variableDomainsDict.items():
                returnString += (
                    str(key) + " : " + " ".join([value for value in domain]) + "\n"
                )
            returnString += '"""\n\n'
        ...
        returnString += " ".join(
            [conditionedVariable for conditionedVariable in self.__conditionedVariables]
        )
        returnString += '"\n\n'
        ...
        for assignmentDict in self.getAllPossibleAssignmentDicts():
            ...
            returnString += ", ".join(
                [
                    variable + " : " + str(assignmentDict[variable])
    ...
    def specializeVariableDomains(self, newVariableDomainsDict):
            ...
            if variable in self.variablesSet():
                ...
                for value in domain:
                    if value not in oldVariableDomain:
                        raise ValueError(
                            "newVariableDomainsDict is not a subset of factor.variableDomainsDict ",
                            "for variables contained in factor. "
                            + "factor: "
                            + str(self)
                            + " newVariableDomainsDict: "
                            + str(newVariableDomainsDict)
                            + " factor.variableDomainsDict: "
                            + str(self.variableDomainsDict())
                            + " variable: "
                            + str(variable)
                            + " value: "
                            + str(value),
        ...
        for assignmentDict in newFactor.getAllPossibleAssignmentDicts():
            newFactor.setProbability(
                assignmentDict, self.getProbability(assignmentDict)
            )
...
def fillTablesRandomly(bayesNet):
    ...
    for variable in bayesNet.variablesSet():
        ...
        CPT = constructAndFillFactorRandomly(
            [variable], conditionedVariablesList, bayesNet.variableDomainsDict()
        )
        bayesNet.setCPT(variable, CPT)


def fillWithOneConditionedAssignmentRandomly(
    factor, unconditionedVariables, conditionedVariables, product, variableDomainsDict
):
    """
    Fills one subtable of a factor (given one conditional assignment).
    Makes this subtable sum to 1.
    """
    cartesianProductOfUnConditionalAssignments = itertools.product(
        *[
            variableDomainsDict[unconditionedVariable]
            for unconditionedVariable in unconditionedVariables
        ]
    )
    randomFills = [
        max(0.0, random.uniform(-0.4, 0.8))
        for variableValue in cartesianProductOfUnConditionalAssignments
    ]
    ...
    for (randomFill, variableValue) in zip(
        randomFills, cartesianProductOfUnConditionalAssignments
    ):
        assignmentDict = dict(
            zip(
                list(unconditionedVariables) + list(conditionedVariables),
                list(variableValue) + list(product),
            )
        )
        factor.setProbability(assignmentDict, randomFill / conditionalProbabilitySum)


def constructAndFillFactorRandomly(
    ...
    if len(conditionedVariables) > 0:
        cartesianProductOfConditionalAssignments = itertools.product(
            *[
                variableDomainsDict[conditionedVariable]
                for conditionedVariable in conditionedVariables
            ]
        )
        for product in cartesianProductOfConditionalAssignments:
            fillWithOneConditionedAssignmentRandomly(
                newFactor,
        ...
        fillWithOneConditionedAssignmentRandomly(
            newFactor, unconditionedVariables, [], [], variableDomainsDict
        )
    return newFactor


def reduceBayesNetVariablesWithEvidence(bayesNet, variablesToRemove, evidenceDict):
    """
    ...
    if len(variablesToRemoveSet & evidenceVariables) > 0:
        raise ValueError(
            "Evidence variables are in the list of variable to "
            + "be removed from the Bayes' net.  This is "
            + "undefined. Evidence: "
            + str(evidenceDict)
            + ". Variables to remove: "
            + str(variablesToRemoveSet)
        )
    ...
    for variable in newVariables:
        newOutEdges[variable] = set(
            [y for y in oldOutEdges[variable] if y in newVariables]
        )
        newInEdges[variable] = set(
            [y for y in oldInEdges[variable] if y in newVariables]
        )

    newVariableDomainsDict = bayesNet.getReducedVariableDomains(evidenceDict)

    newBayesNet = BayesNet(
        newVariables, newInEdges, newOutEdges, newVariableDomainsDict
    )
    ...
    for variable in bayesNet.variablesSet():
        if variable in newVariables:
            ...
            if (
                variable in evidenceVariables
                and len(unconditionedVariablesParents) == 0
            ):
                newCPT = Factor(
                    [variable], evidenceVariablesParents, newVariableDomainsDict
                )
...
def printStarterBayesNet():
    ...
    print(
        "Print a conditional probability table (henceforth known as a CPT) "
    ...
    print(
        "Note that in the table output of print for factors with conditioned "
    ...
    print(
        "You can use factor.getAllPossibleAssignmentDicts() "
    ...
    print(
        "Print a Bayes' net to see its variables, edges, and "
        + "the CPT for each variable.\n"
    )
    print(bayesNet)

    print(
        "You can get a list of all CPTs from a Bayes' net, instantiated with "
    ...
    print(
        "If only one variable's CPT is desired, you can get just that particular "
...
def normalize(factor):
    ...
    probSum = sum(
        [factor.getProbability(a) for a in factor.getAllPossibleAssignmentDicts()]
    )
    if not probSum:
        return None
