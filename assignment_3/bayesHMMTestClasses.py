...
class GraphEqualityTest(testClasses.TestCase):
    ...
    def execute(self, grades, moduleDict, solutionDict):
        ...
        goldNet = bayesNet.constructEmptyBayesNetFromString(
            solutionDict["solutionString"]
        )
        ...
        if missingVars:
            self.addMessage(
                "Student solution is missing variables: " + str(missingVars) + "\n"
            )
        if extraVars:
            self.addMessage(
                "Student solution has extra variables: " + str(extraVars) + "\n"
            )
        studentEdges = set(
            [
                str(fromVar) + " -> " + str(toVar)
                for toVar in studentNet.variablesSet()
                for fromVar in studentNet.inEdges()[toVar]
            ]
        )
        goldEdges = set(
            [
                str(fromVar) + " -> " + str(toVar)
                for toVar in goldNet.variablesSet()
                for fromVar in goldNet.inEdges()[toVar]
            ]
        )
        ...
        if not sameValues:
            ...
            if missingDictVars:
                self.addMessage(
                    "Student dictionary is missing variables: " + str(missingDictVars)
                )
            if extraDictVars:
                self.addMessage(
                    "Student dictionary has extra variables: " + str(extraDictVars)
                )
    ...
    def writeSolution(self, moduleDict, filePath):
        ...
        with open(filePath, "w") as handle:
            handle.write(
                '# This is the solution file for %s.\n\nsolutionString: """\n'
                % self.path
            )
...
class BayesNetEqualityTest(GraphEqualityTest):
    def execute(self, grades, moduleDict, solutionDict):
        ...
        if not studentNet.sameGraph(goldNet):
            self.addMessage(
                "Bayes net graphs are not equivalent. Please check that your Q1 implementation is correct."
            )
            return self.testFail(grades)
        ...
        for variable in goldNet.variablesSet():
            try:
                ...
                self.addMessage(
                    "Student Bayes net missing CPT for variable " + str(variable)
                )
                return self.testFail(grades)
            goldFactor = goldNet.getCPT(variable)
            if not studentFactor == goldFactor:
                self.addMessage(
                    "First factor in which student answer differs from solution: P({} | {})".format(
                        studentFactor.unconditionedVariables(),
                        studentFactor.conditionedVariables(),
                    )
                )
...
class FactorEqualityTest(testClasses.TestCase):
        ...
        if studentFactor == goldenFactor:
            ...
            if self.alg == "inferenceByVariableElimination":
                ...
                if self.callTrackingList != goldenCallTrackingList:
                    self.addMessage(
                        "Order of joining by variables and elimination by variables is incorrect for variable elimination"
                    )
                    self.addMessage(
                        "Student performed the following operations in order: "
                        + str(self.callTrackingList)
                        + "\n"
                    )
                    self.addMessage(
                        "Correct order of operations: "
                        + str(goldenCallTrackingList)
                        + "\n"
                    )
                    return self.testFail(grades)
            ...
            self.addMessage(
                "Student generated factor:\n\n" + str(studentFactor) + "\n\n"
            )
            self.addMessage("Correct factor:\n\n" + str(goldenFactor) + "\n")

            studentProbabilityTotal = sum(
                [
                    studentFactor.getProbability(assignmentDict)
                    for assignmentDict in studentFactor.getAllPossibleAssignmentDicts()
                ]
            )
            correctProbabilityTotal = sum(
                [
                    goldenFactor.getProbability(assignmentDict)
                    for assignmentDict in goldenFactor.getAllPossibleAssignmentDicts()
                ]
            )
            if abs(studentProbabilityTotal - correctProbabilityTotal) > 10e-12:
                self.addMessage(
                    "Sum of probability in student generated factor is not the same as in correct factor"
                )
                self.addMessage(
                    "Student sum of probability: " + str(studentProbabilityTotal)
                )
                self.addMessage(
                    "Correct sum of probability: " + str(correctProbabilityTotal)
                )
    ...
    def writeSolution(self, moduleDict, filePath):
        ...
        if self.constructRandomly:
            if (
                self.alg == "joinFactors"
                or self.alg == "eliminate"
                or self.alg == "normalize"
            ):
                replaceTestFile(self.testPath, "Factors", self.factorsDict)
            elif (
                self.alg == "inferenceByVariableElimination"
                or self.alg == "inferenceByLikelihoodWeightingSampling"
            ):
        ...
        with open(filePath, "w") as handle:
            ...
            if self.alg == "inferenceByVariableElimination":
                handle.write(
                    'callTrackingList: "' + repr(self.callTrackingList) + '"\n'
                )
        return True
...
class FactorInputFactorEqualityTest(FactorEqualityTest):
    def __init__(self, question, testDict):
        ...
        parseDict = parseFactorInputProblem(
            testDict,
            goingToEliminate=eliminateToPerform,
    ...
    def solveProblem(self, moduleDict):
        ...
        if self.alg == "joinFactors":
            ...
            solvedFactor = studentComputation(
                list(self.factorsDict.values())[0], self.eliminateVariable
            )
            ...
            specializedFactor = origFactor.specializeVariableDomains(
...
class BayesNetInputFactorEqualityTest(FactorEqualityTest):
    ...
    def solveProblem(self, moduleDict):
        inferenceModule = moduleDict["inference"]
        if self.alg == "inferenceByVariableElimination":
            studentComputationWithCallTracking = getattr(
                inferenceModule, self.alg + "WithCallTracking"
            )
            studentComputation = studentComputationWithCallTracking(
                self.callTrackingList
            )
            solvedFactor = studentComputation(
                self.problemBayesNet,
                self.queryVariables,
                self.evidenceDict,
                self.variableEliminationOrder,
            )
        elif self.alg == "inferenceByLikelihoodWeightingSampling":
            randomSource = util.FixedRandom().random
            studentComputationRandomSource = getattr(
                inferenceModule, self.alg + "RandomSource"
            )
...
class MostLikelyFoodHousePositionTest(testClasses.TestCase):
    ...
    def execute(self, grades, moduleDict, solutionDict):
        ...
        studentPosition = studentFunction(
            self.evidence, studentBayesNet, self.eliminationOrder
        )[FOOD_HOUSE_VAR]
    ...
    def writeSolution(self, moduleDict, filePath):
        ...
        answer = staffFunction(self.evidence, staffBayesNet, self.eliminationOrder)[
            FOOD_HOUSE_VAR
        ]
        with open(filePath, "w") as handle:
            handle.write(
...
class VPITest(testClasses.TestCase):
    ...
    def execute(self, grades, moduleDict, solutionDict):
        ...
        studentAnswer = eval(
            "studentAgent.{}(self.evidence, self.eliminationOrder)".format(
                self.targetFunction
            )
        )
        ...
        if type(studentAnswer) == float:
            ...
            correct = closeNums(studentAnswer[0], goldAnswer[0]) & closeNums(
                studentAnswer[1], goldAnswer[1]
            )
        if not correct:
    ...
    def writeSolution(self, moduleDict, filePath):
        ...
        answer = eval(
            "agent.{}(self.evidence, self.eliminationOrder)".format(self.targetFunction)
        )
        with open(filePath, "w") as handle:
            handle.write(
                '# This is the solution file for %s.\n\nanswer: """\n' % self.path
...
def parseFactorInputProblem(
    testDict, goingToEliminate=False, goingToEvidenceAssign=False
):
    parseDict = {}
    variableDomainsDict = {}
    ...
    factorsDict = (
        {}
    )  # assume args is a list of factor names and maybe a variable name at the end
    if goingToEliminate:
    ...
    for factorName in testDict["factorArgs"].split(" "):
        ...
        currentFactor = parseFactorFromFileDict(
            testDict, variableDomainsDict=variableDomainsDict, prefix=factorName
        )
    ...
    with open(abs_path, "w") as new_file:
        ...
        if typeOfTest == "BayesNet":
            ...
            new_file.write(
                ...
                + "\n".join(
                    [
                        factor.easierToParseString(
                            prefix=name, printVariableDomainsDict=False
                        )
                        for name, factor in inputToTest.items()
                    ]
                )
            )
...
def parseSolutionBayesNet(solutionDict):
    ...
    for variable in variables:
        net.setCPT(
            variable,
            parseFactorFromFileDict(solutionDict, variableDomainsDict, variable),
        )
...
def parseBayesNetProblem(testDict):
    ...
    if testDict["constructRandomly"] == "False":
        ...
        for variable in variables:
            currentFactor = bayesNet.Factor(
                [variable], problemBayesNet.inEdges()[variable], variableDomainsDict
            )
...
class DoubleInferenceAgentTest(testClasses.TestCase):
    ...
    def execute(self, grades, moduleDict, solutionDict):
        ...
        if self.inference == "ParticleFilter":
            for pfilter in pac.inferenceModules:
                pfilter.setNumParticles(self.numParticles)
        elif self.inference == "MarginalInference":
            ...
            if isinstance(inference, ParticleFilter):
                if len(inference.particles) != inference.numParticles:
                    t = (
                        ...
                        len(inference.particles),
                        inference.numParticles,
                    )
                    summary = (
                        "%s) Filters do not have the same number of particles.\n\tstudent count: %d\n\treference count: %d"
                        % t
                    )
