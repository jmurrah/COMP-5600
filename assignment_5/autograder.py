        ...
        assert all(
            [
                ...
                for (actual, expected) in zip(
                    node.detach().numpy().shape, expected_shape
                )
            ]
...
def check_perceptron(tracker):
    ...
    for dimensions in range(1, 10):
        ...
        for param in p.parameters():
            ...
            verify_node(
                param, "parameter", (1, dimensions), "PerceptronModel.parameters()"
            )
    ...
    for multiplier in (-5, -2, 2, 5):
        p = models.PerceptronModel(dimensions)
        orig_weights = (
            p.get_weights().data.reshape((1, dimensions)).detach().numpy().copy()
        )
...
def check_regression(tracker):
    ...
    assert np.isclose(train_loss, sanity_loss), (
        ...
        "based on the output of RegressionModel()".format(train_loss, sanity_loss)
    )
...
def check_lang_id(tracker):
    ...
    for batch_size, word_length in ((1, 1), (2, 1), (2, 6), (4, 8)):
        ...
        for gradient in grad:
            assert (
                gradient != None
            ), "Output returned from LanguageIDModel.run() does not depend on all of the provided inputs (xs)"
...
def check_convolution(tracker):
    ...
    for matrix_size in (
        ...
        4,
        6,
    ):  # Test 3 random convolutions to test convolve() function
        ...
        ).all(), "The convolution returned by Convolve() does not match expected output"
...
def check_attention(tracker):
    ...
    for block_size in [2, 4, 16]:
        ...
        with torch.no_grad():
            ...
            att_block.q_layer.weight = torch.nn.Parameter(
                torch.ones((layer_size, layer_size)).reshape(layer_size, layer_size)
                * q_weight
            )
            att_block.v_layer.weight = torch.nn.Parameter(
                torch.ones((layer_size, layer_size)).reshape(layer_size, layer_size)
                * v_weight
            )
        ...
        expected_output = (
            torch.matmul(
                att_block.k_layer(input), torch.movedim(att_block.q_layer(input), 1, 2)
            )
            / layer_size**0.5
        )

        expected_output = expected_output.masked_fill(
            att_block.mask[:, :, :T, :T] == 0, float("-inf")
        )[0]

        output = torch.matmul(
            nn.functional.softmax(expected_output, dim=-1), att_block.v_layer(input)
        )

        assert torch.all(
            torch.isclose(output, att_block(input))
        ), "The output returned by Attention() does not match expected output"
